...
student * findstudent(vector<student> &allstudents, string key){ //[Missing Code 4] There is something wrong in this line.
	...
	while(getline(course_file,textline)){
		if(state == 1){
			...
			if(textline == "---------------------------------------"){
				...
				student *p = findstudent(allstudents,(textline.c_str()));
